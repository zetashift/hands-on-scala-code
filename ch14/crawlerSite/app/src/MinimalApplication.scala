...
object MinimalApplication extends cask.MainRoutes {
        ...
        div(cls := "container")(
          ...
          form(onsubmit := "submitForm(); return false")(
            ...
            input(
              `type` := "text",
              id := "depthInput",
              placeholder := "Enter a search depth"
            ),
            input(`type` := "submit")
          ),
          div(id := "resultDiv"),
  ...
  def subscribe() = cask.WsHandler { connection =>
    cask.WsActor { case cask.Ws.Text(s"$depth0 $searchTerm") =>
      fetchAllLinksParallel(
              ...
              div(
                results.toSeq
                  .flatMap(s =>
                    Seq[Frag](
                      " - ",
                      a(href := s"https://en.wikipedia.org/wiki/$s")(s)
                    )
                  )
                  .drop(1)
              ).render
        ...
        onDepth =
          i => connection.send(cask.Ws.Text(div(b("Depth ", i + 1)).render))
      )
    }
  }
