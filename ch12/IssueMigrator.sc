@main def main(srcRepo: String, destRepo: String) = {
    ...
    for (comment <- comments)
      yield (
        comment("issue_url").str match {
          case s"https://api.github.com/repos/zetashift/$repo/issues/$id" =>
            id.toInt
        },
  ...
  val issueNums =
    for ((number, title, body, user) <- issueData.sortBy(_._1)) yield {
  ...
  for (
    (issueId, user, body) <- commentData; newIssueId <- issueNumMap.get(issueId)
  ) {
    println(s"Commenting on issue old_id=$issueId new_id=$newIssueId")
    val resp = requests.post(
      s"https://api.github.com/repos/zetashift/test/issues/$newIssueId/comments",
